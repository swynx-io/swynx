...
export function renderActionListMarkdown(actionList) {
    ...
    for (const [category, catIssues] of Object.entries(byCategory)) {
      ...
      for (const issue of catIssues) {
        ...
        if (issue.exploitable !== undefined) {
          lines.push(`  - Actually exploitable: ${issue.exploitable ? 'YES' : 'NO'}${issue.usageLocations?.length ? ` — called at \`${issue.usageLocations[0]}\`` : ''}`);
        }
  ...
  if (quickWins?.length > 0) {
    ...
    for (const issue of quickWins) {
      const impact = issue.impact?.cost
        ? `£${issue.impact.cost.toFixed(0)}/year`
        : issue.impact?.bytes
  ...
  // Footer
  lines.push(`*Generated by Swynx v${meta.swynxVersion || meta.peerAuditVersion || '1.0.0'}*`);
...
export function renderProgressMarkdown(progressReport) {
  ...
  lines.push(`**Health Score:** ${meta.previousScan.healthScore} → ${meta.currentScan.healthScore} (${healthSymbol}${meta.healthChange})`);
  lines.push('');
  lines.push('---');
  lines.push('');
  ...
  lines.push(`| ✗ Still Open | ${summary.stillOpen} |`);
  ...
  if (nextActions?.length > 0) {
    ...
    for (let i = 0; i < nextActions.length; i++) {
      ...
      lines.push(`${i + 1}. **${issue.title}** — ${reason}${issue.fix?.effort ? `, ${issue.fix.effort} effort` : ''}`);
    }
