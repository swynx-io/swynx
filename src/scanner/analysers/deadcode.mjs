...
export async function findDeadCode(jsAnalysis, importGraph, projectPath = null, packageJson = {}, config = {}, onProgress = () => {}) {
  ...
  // ── JavaScript/TypeScript dead function detection ─────────────────────
  // Two compiler/language-enforced private scopes:
  //   1. Module-level functions that are never exported — in ESM/CJS these are
  //      file-scoped by the language and can only be called within their file.
  //   2. Private class methods — ES #private (engine-enforced file-private) and
  //      TS `private` (compiler-enforced; `as any` escapes caught by the
  //      project-wide fallback search below).
  // Same approach as Go: strip the function's own body, then word-boundary
  // search the remaining file content. Self-recursion doesn't count as usage.
  const jsFnRegex = /\.([mc]?js|[mc]?ts|jsx|tsx)$/;
  const jsCandidateExcludeRe = /\.d\.[mc]?ts$|\.min\.[mc]?js$|(?:^|\/)(?:dist|build|out|vendor|third_party|coverage|__snapshots__|__mocks__|__fixtures__|fixtures?|examples?|node_modules)\/|(?:^|\/)(?:tests?|__tests__|spec|e2e|cypress|playwright)\/|\.(?:spec|test|stories|cy)\.[mc]?[jt]sx?$|(?:^|\/)(?:tests?|specs?)\.[mc]?[jt]sx?$|_(?:test|spec)\.[mc]?[jt]sx?$|[\w.-]*\.config\.[mc]?[jt]s$|(?:^|\/)(?:babel|metro|jest|webpack|rollup|vite|next|nuxt|tailwind|postcss)\.config\./;
  const jsGeneratedRe = /@generated|DO NOT EDIT|Code generated|auto-?generated|autogenerated/i;
  // Files using runtime escape hatches can reach "private" functions dynamically
  const jsEscapeHatchRe = /\beval\s*\(|\bnew\s+Function\s*\(|\brewire\b|__get__\s*\(|__GetDependency__/;

  // Collect all searchable JS/TS files once (for the project-wide fallback pass)
  const jsAllFiles = [];
  const jsPrivateMethodSurvivors = [];  // TS `private` methods needing project-wide confirmation
  for (const file of analysisFiles) {
    const fp = file.file?.relativePath || file.file;
    if (jsFnRegex.test(fp) || fp.endsWith('.vue') || fp.endsWith('.svelte')) jsAllFiles.push(fp);
  }

  const jsFnDebug = process.env.SWYNX_DEBUG_JSFN ? { ext: 0, reach: 0, excl: 0, parse: 0, nofns: 0, scanned: 0 } : null;
  for (const file of analysisFiles) {
    const filePath = file.file?.relativePath || file.file;
    if (!jsFnRegex.test(filePath)) { if (jsFnDebug) jsFnDebug.ext++; continue; }               // .vue/.svelte excluded: line numbers are script-block relative
    if (!reachableFiles.has(filePath)) { if (jsFnDebug) jsFnDebug.reach++; continue; }
    if (jsCandidateExcludeRe.test(filePath)) { if (jsFnDebug) jsFnDebug.excl++; continue; }
    if (file.parseMethod && file.parseMethod !== 'babel-ast') { if (jsFnDebug) jsFnDebug.parse++; continue; }  // need exact AST line boundaries
    if (jsFnDebug) jsFnDebug.scanned++;

    const fns = file.functions || [];
    if (fns.length === 0) continue;

    let jsContent;
    try { jsContent = readFileSync(join(projectPath, filePath), 'utf-8'); } catch { continue; }

    const jsLines = jsContent.split('\n');
    // Minified / generated / escape-hatch files: skip entirely
    if (jsLines.length > 0 && jsContent.length / jsLines.length > 300) continue;
    if (jsGeneratedRe.test(jsContent.slice(0, 1500))) continue;
    if (jsEscapeHatchRe.test(jsContent)) continue;

    // Only analyse real modules. In a classic <script> file, top-level functions
    // are globals callable from HTML attributes or other scripts.
    const isModule = (file.imports && file.imports.length > 0) ||
                     (file.exports && file.exports.length > 0) ||
                     /^\s*(?:import|export)\b/m.test(jsContent) ||
                     /\b(?:require\s*\(|module\.exports|exports\.[\w$]+\s*=)/.test(jsContent);
    if (!isModule) continue;

    const isTs = /\.[mc]?tsx?$/.test(filePath);
    const isAngularComponent = /@(?:Component|Directive|Injectable|Pipe)\s*\(/.test(jsContent);

    // Dynamic member dispatch on `this` (this[`_handle${x}`], this[name]) makes
    // private-method analysis unsafe for this file.
    const hasDynamicThisAccess = /this\s*\[\s*[^'"`\]]/.test(jsContent) || /this\s*\[\s*`[^`]*\$\{/.test(jsContent);

    // Names dynamically constructed anywhere in the file (obj['prefix' + x]) — collect prefixes
    const jsDynamicPrefixes = new Set();
    let jdm;
    const bracketConcatRe = /\[\s*['"]([\w$]{3,})['"]\s*\+/g;
    while ((jdm = bracketConcatRe.exec(jsContent)) !== null) jsDynamicPrefixes.add(jdm[1]);
    const bracketTemplateRe = /\[\s*`([\w$]{3,})\$\{/g;
    while ((jdm = bracketTemplateRe.exec(jsContent)) !== null) jsDynamicPrefixes.add(jdm[1]);

    for (const fn of fns) {
      if (!fn.name || fn.name.length <= 1 || fn.name === '_' || fn.name === 'anonymous') continue;
      if (!fn.line || !fn.endLine || fn.endLine < fn.line) continue;
      if (fn.decorators && fn.decorators.length > 0) continue;  // framework-invoked

      const isEsPrivate = fn.isPrivateName || fn.name.startsWith('#');
      const isTsPrivate = fn.type === 'method' && fn.accessibility === 'private';
      const isModuleLocal = !fn.className && !fn.isObjectMethod && !fn.isObjectProperty &&
                            (fn.type === 'function' || fn.type === 'arrow' || fn.type === 'expression') &&
                            !fn.exported;

      let candidateKind = null;
      if (isEsPrivate || isTsPrivate) {
        if (fn.kind && fn.kind !== 'method') continue;        // skip get/set/constructor
        if (hasDynamicThisAccess) { if (jsFnDebug) jsFnDebug.dynThis = (jsFnDebug.dynThis || 0) + 1; continue; }
        if (isTsPrivate && isAngularComponent) continue;      // legacy Angular templates can call private members
        candidateKind = 'private-method';
      } else if (isModuleLocal) {
        candidateKind = 'module-local';
      } else {
        continue;
      }
      if (jsFnDebug) jsFnDebug[candidateKind] = (jsFnDebug[candidateKind] || 0) + 1;

      const bareName = fn.name.replace(/^#/, '');
      if (bareName.length <= 1) continue;
      // Skip names matched by dynamic construction prefixes
      let jsDynHit = false;
      for (const prefix of jsDynamicPrefixes) {
        if (bareName.startsWith(prefix)) { jsDynHit = true; break; }
      }
      if (jsDynHit) continue;

      // Lookarounds, not \b: word boundaries fail on $-prefixed names
      // (`.` → `$` is not a boundary, so `this.$foo()` would never match \b\$foo)
      const jsEscaped = bareName.replace(/\$/g, '\\$');
      const nameRe = isEsPrivate
        ? new RegExp(`#${jsEscaped}(?![\\w$])`)
        : new RegExp(`(?<![\\w$#])${jsEscaped}(?![\\w$])`);

      // Strip the function's own body (declaration included) and search the rest
      const startIdx = fn.line - 1;
      const endIdx = Math.min(fn.endLine - 1, jsLines.length - 1);
      const outside = jsLines.filter((_, i) => i < startIdx || i > endIdx).join('\n');
      if (nameRe.test(outside)) continue;

      const sizeBytes = jsLines.slice(startIdx, endIdx + 1).join('\n').length;
      const record = {
        name: fn.name,
        file: filePath,
        line: fn.line,
        ...
        reason: candidateKind === 'module-local' ? 'module-local-never-called' : 'private-never-called',
        verdict: 'unreachable',
        cwe: 'CWE-561',
        evidence: { cwe: 'CWE-561', scope: candidateKind === 'module-local' ? 'file-private' : 'class-private', searchedFiles: 1, method: 'body-strip-word-boundary-search' }
      };

      if (isTsPrivate) {
        // TS `private` is compiler-enforced but escapable via `(x as any)._m()` —
        // confirm with a project-wide search before reporting.
        jsPrivateMethodSurvivors.push({ record, nameRe });
      } else {
        // ES #private and module-local functions are engine-enforced file-private
        results.deadFunctions.push(record);
        results.summary.totalDeadFunctions++;
        results.summary.totalDeadFunctionBytes += sizeBytes;
      }
    }
  ...
  if (jsFnDebug) console.error('[jsfn-debug]', JSON.stringify(jsFnDebug), 'survivors:', jsPrivateMethodSurvivors.length);

  // Project-wide confirmation pass for TS private-method survivors (single sweep)
  if (jsPrivateMethodSurvivors.length > 0) {
    const pending = [...jsPrivateMethodSurvivors];
    for (const fp of jsAllFiles) {
      if (pending.length === 0) break;
      let otherContent;
      try { otherContent = readFileSync(join(projectPath, fp), 'utf-8'); } catch { continue; }
      for (let i = pending.length - 1; i >= 0; i--) {
        if (fp === pending[i].record.file) continue;  // declaring file already checked
        pending[i].nameRe.lastIndex = 0;
        if (pending[i].nameRe.test(otherContent)) pending.splice(i, 1);  // referenced elsewhere — not dead
      }
    }
    for (const { record } of pending) {
      record.evidence.searchedFiles = jsAllFiles.length;
      results.deadFunctions.push(record);
      results.summary.totalDeadFunctions++;
      results.summary.totalDeadFunctionBytes += record.sizeBytes;
    }
  }
  ...
  // Publishable-package gate: in a library package, exports with no internal
  // importers are usually PUBLIC API consumed by downstream users, not dead
  // code. Only analyse exports in private (non-published) packages.
  function isPublishableSurface(pkg) {
    if (!pkg || pkg.private === true) return false;
    return !!(pkg.main || pkg.module || pkg.exports || pkg.types || pkg.typings || pkg.bin || pkg.files || pkg.publishConfig);
  }
  const exportPkgDirs = [...findNestedPackageJsons(projectPath).entries()]
    .map(([dir, pkg]) => [dir, isPublishableSurface(pkg)])
    .sort((a, b) => b[0].length - a[0].length);
  const rootIsPublishable = isPublishableSurface(packageJson);
  function isInPublishablePackage(fp) {
    for (const [dir, publishable] of exportPkgDirs) {
      if (fp.startsWith(dir + '/')) return publishable;
    }
    return rootIsPublishable;
  }

  // Exports consumed by framework convention, not by imports — never flag these
  const frameworkExportNames = new Set([
    // Next.js app/pages router
    'GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS',
    'middleware', 'config', 'metadata', 'viewport', 'revalidate', 'dynamic',
    ...
    'getServerSideProps', 'getStaticProps', 'getStaticPaths', 'getInitialProps', 'reportWebVitals',
    // Remix / React Router
    'loader', 'action', 'links', 'handle', 'meta', 'shouldRevalidate', 'ErrorBoundary', 'HydrateFallback', 'headers',
    // SvelteKit
    'load', 'prerender', 'csr', 'ssr', 'trailingSlash', 'entries', 'actions',
    // Astro / Cloudflare / misc
    'onRequest',
  ]);
  const exportSkipFileRe = /\.(?:stories|story|spec|test|cy)\.[mc]?[jt]sx?$|(?:^|\/)(?:tests?|__tests__|__mocks__|spec|e2e|cypress|playwright)\//;
  ...
  for (const file of analysisFiles) {
    ...
    if (entryPointFiles.has(filePath)) continue;
    // Skip files in publishable packages — their exports may be public API
    if (/\.[mc]?[jt]sx?$/.test(filePath) && isInPublishablePackage(filePath)) continue;
    // Skip test/story files — their exports are consumed by tooling, not imports
    if (exportSkipFileRe.test(filePath)) continue;
    ...
    for (const exp of fileExportsList) {
      ...
      // Framework-convention exports (route handlers, loaders, page options)
      // are invoked by the framework, never imported — treat as live
      if (frameworkExportNames.has(exportName)) continue;
    ...
    // Defer reporting until the project-wide occurrence veto below has run
    pendingPartials.push({ file, filePath, liveExports, deadExports });
  }

  // Project-wide occurrence veto: import tracking misses consumption via
  // workspace package names ('@scope/pkg'), default-object property access
  // (`axios.toFormData`), and string-based registration. If a flagged export's
  // name appears ANYWHERE else in the codebase, drop it — accuracy over recall.
  if (pendingPartials.length > 0) {
    const vetoCandidates = [];
    for (const p of pendingPartials) {
      for (const exp of p.deadExports) {
        exp._vetoed = exp.name === 'default';  // 'default' matches everywhere; always unverifiable
        if (!exp._vetoed) {
          vetoCandidates.push({
            pending: p,
            exp,
            re: new RegExp(`(?<![\\w$#])${exp.name.replace(/\$/g, '\\$')}(?![\\w$])`)
          });
        }
      }
    }
    if (vetoCandidates.length > 0) {
      const unvetoed = [...vetoCandidates];
      for (const otherFile of analysisFiles) {
        if (unvetoed.length === 0) break;
        const otherPath = otherFile.file?.relativePath || otherFile.file;
        let otherContent;
        try { otherContent = readFileSync(join(projectPath, otherPath), 'utf-8'); } catch { continue; }
        if (otherContent.length > 2_000_000) continue;
        for (let i = unvetoed.length - 1; i >= 0; i--) {
          if (otherPath === unvetoed[i].pending.filePath) continue;  // own file already tracked by import graph
          if (unvetoed[i].re.test(otherContent)) {
            unvetoed[i].exp._vetoed = true;
            unvetoed.splice(i, 1);
          }
        }
      }
  ...
  for (const pending of pendingPartials) {
    const { file, filePath, liveExports } = pending;
    const deadExports = pending.deadExports.filter(e => !e._vetoed);
    for (const e of pending.deadExports) delete e._vetoed;
